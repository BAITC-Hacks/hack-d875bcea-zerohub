...
const source = mockAI ? "ai" : "rules";
const reportLabel = mockAI ? "AI explanation of the calculated scenario" : "Rule-based explanation · no AI model was called";
...
const child = spawn(
  ...
  [
    "-m",
    "uvicorn",
    ...(mockAI ? ["tests.person4.mock_app:make_app", "--factory"] : ["app.main:app"]),
    "--host",
    "127.0.0.1",
    "--port",
    String(port),
...
try {
  ...
  const over = structuredClone(body);
  over.decisions[0].initiative_id = "transport_junction";
  const rejected = await fetch(base + "/scenarios", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(over)});
  assert.equal(rejected.status, 422, "Over-budget scenario must be rejected");
  ...
  console.log(`PASS (${mockAI ? "mocked AI HTTP provider" : "rules"}): API schemas and budget rejection.`);
  console.log(
